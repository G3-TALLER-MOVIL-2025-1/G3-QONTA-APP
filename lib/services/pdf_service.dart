...
class PdfService {
  ...
  Future<Uint8List> createInvoice(List<Transaction> transactions) async {
    final pdf = pw.Document();

    final List<Transaction> elements = [
      Transaction(amount: "Cantidad", category: "Categoría", description: "Descripción", date: "Fecha"),
      for (var transaction in transactions)
      Transaction(amount: transaction.amount, category: transaction.category, description: transaction.description, date: transaction.date),
        // CustomRow(
        //   product.name,
  ...
  pw.Expanded itemColumn(List<Transaction> elements) {
    return pw.Expanded(
      child: pw.Column(
        children: [
          for (var element in elements)
            pw.Row(
              children: [
                ...
                pw.Expanded(
                    child:
                        pw.Text(element.description, textAlign: pw.TextAlign.right)),
                pw.Expanded(
                    child:
                        pw.Text(element.date!, textAlign: pw.TextAlign.right)),
                // pw.Expanded(
  ...
  Future<void> savePdfFile(String fileName, Uint8List byteList) async {
    ...
    final output = Platform.isAndroid ? await getExternalStorageDirectory()
  ...
  // String getSubTotal(List<Transaction> products) {
  //   return products
  //       .fold(0.0,
